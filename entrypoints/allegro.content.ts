	...
	main() {
		...
		const analyse = (): void => {
			...
			timer = setTimeout(() => {
				...
				if (import.meta.env.DEV && state.diagnostics)
					console.debug("[Dopakuj] diagnostyka parsera", state.diagnostics);
			}, 250);
		};
		...
		chrome.runtime.onMessage.addListener(
			(message: ExtensionMessage, _sender, sendResponse) => {
				...
				if (message.type === "GET_PUBLIC_PAGE_HTML") {
					...
					const blocked =
						/captcha|nie jesteś robotem|nietypowy ruch|access denied|odmowa dostępu|please enable js and disable any ad blocker/.test(
							visibleText,
						);
					const empty = /brak ofert|nie znaleźliśmy ofert|0 ofert/.test(
						visibleText,
					);
					const ready =
						document.querySelectorAll(
							'a[href*="/oferta/"], a[href*="/produkt/"][href*="offerId="]',
						).length > 0;
					sendResponse({
						ok: true,
