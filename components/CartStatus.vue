...
<template>
	<section
		class="cart-status"
		:class="{ 'status-error': state.status === 'error' }"
		aria-live="polite"
	>
		...
		<div class="status-copy">
			<span v-if="state.status === 'ready'"
				>Koszyk <span class="separator">·</span>
				<strong
			...
			<span v-else-if="state.status === 'empty'"
				>Koszyk jest pusty. Dodaj pierwszy produkt.</span
			>
			<span v-else-if="state.status === 'error'">{{
				state.error || "Nie udało się odczytać koszyka."
			}}</span>
			<span v-else>Przejdź do koszyka.</span>
		</div>
		<button
			v-if="context.isCart || state.sourceTabId !== undefined"
			class="icon-button ghost"
			type="button"
			:disabled="busy || state.status === 'loading'"
			title="Odśwież koszyk"
			aria-label="Odśwież dane koszyka"
			@click="emit('refresh')"
		>
			<RefreshCw :size="15" />
		</button>
